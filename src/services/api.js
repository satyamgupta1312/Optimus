...
// This file is intended to handle communication with the backend.

export const GET_PAGE_SKELETON = "/api/app/page_skeleton/v2/";
...
export const fetchWidgetProductList = async (widgetId, itemId, page = 1) => {
    try {
        const url = `/api/app/get_paginated_widget_product_list/v2/?widget_id=${widgetId}&item_id=${itemId || ''}&cart_value=0.0&store_id=166&page_no=${page}&limit=20`;
...
export const fetchMediaBySlug = async (slug) => {
    try {
        ...
        const url = `/api/app/widget/?slug_name=${slug}`;
